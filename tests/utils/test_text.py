...
class TestTwoBraceTemplate:
    ...
    def test_various_valid_patterns(self, template_str, kwargs, expected):
        template = TwoBraceTemplate(template_str)
        assert template.substitute(**kwargs) == expected

    def test_escaped_braces(self):
        # Test escaping double braces - use {{{{ to output literal {{
        template = TwoBraceTemplate("Use {{{{ to escape braces, like {{{{name}}")
        assert template.substitute(name="foo") == "Use {{ to escape braces, like {{name}}"

    def test_no_substitution_needed(self):
        template = TwoBraceTemplate("No variables here!")
        assert template.substitute() == "No variables here!"

    def test_missing_variable_raises_error(self):
        template = TwoBraceTemplate("Hello {{ name }}!")
