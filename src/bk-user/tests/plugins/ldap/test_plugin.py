...
def ldap_directory_case(request):
    mock_fixture, uuid_attribute = request.param
    request.getfixturevalue(mock_fixture)
    return uuid_attribute


def _set_uuid_attribute(ldap_ds_cfg, id_attribute: str):
    ldap_ds_cfg.data_config.uuid_attribute = id_attribute
...
class TestLDAPDataSourcePlugin:
    def test_get_departments(self, ldap_ds_cfg, logger, ldap_directory_case):
        _set_uuid_attribute(ldap_ds_cfg, ldap_directory_case)
